/*
explanation : here the problem is simple dfs on the 2Xn grid to 
know if there is a path or not .

complexity : dfs complexity
...
int vis[2+9][200000+9],n,k,flag;
char arr[2+9][200000+9];
void dfs(int r,int c,int w)
{
    if(r<1||c<1||r>2||vis[r][c]||c<=w||arr[r][c]=='X')
        return;
