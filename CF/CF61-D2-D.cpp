...
explanation : the idea is that all the edges will be traversed twice anyway 
except for the last node i decide to visit the way to it will be traversed 
...
void dfs(int id,ll w)
{
    ...
    {
        if(!vis[v[id][i].first])
          dfs(v[id][i].first,w+v[id][i].second);
    }
}
int main()
{
