        ...
        for(int h=0;h<events_cnt;h++)
        {
            if(inActiveset[event_h[h].id]==1)
            {
               ...
               else
               {
                   if(--cnt==0)
                   {
                       if(lastt==-1||(rects[event_h[first_rec].id][Entry].y-rects[event_h[lastt].id][Exist].y >1))
                       pri+= deltaX*2,lastt=h;
...
int main()
{
    ...
    */
    for(int i=0;i<n;i++)
    {
        ...
        trects[i][Entry].x=y1; trects[i][Entry].y=x1; trects[i][Exist].x=y2; trects[i][Exist].y=x2;
  ...
  // cout<<ans<<endl;
    for(int i=0;i<n;i++)
    {
        rects[i][Entry].x=trects[i][Entry].x; rects[i][Entry].y=trects[i][Entry].y; rects[i][Exist].x=trects[i][Exist].x;
         rects[i][Exist].y=trects[i][Exist].y;
    }
    sort(event_v,event_v+events_cnt,cmpX);
    sort(event_h,event_h+events_cnt,cmpY);
